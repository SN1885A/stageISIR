...
Coup f_max(Pion* plateau, int profondeur, int x1, int y1, int x2, int y2);
Coup f_min(Pion* plateau, int profondeur, int x1, int y1, int x2, int y2);
...
int f_eval(Pion* jeu) //TODO heuristique
{
	int i, j;
	int valeur=0;
	for (i = 0; i < NB_COLONNES; ++i)
...
Coup f_min(Pion *plateau, int profondeur, int x1, int y1, int x2, int y2){
	...
	min_val.cout = INFINI;
	int result, i, j, k, l;
	int couleur = plateau[i * NB_COLONNES + j].couleur;
	Pion *plateauFictif;
		...
		for (i = 0; i < NB_LIGNES; i++){
			for (j = 0; j < NB_COLONNES; j++){
				for( k=-1; k<=1; k++){
					for( l=-1; l<=1; l++){
						if(k!=0 && l!=0){
							result=f_test_mouvement(plateau, i, j, k, l, couleur);
							if(result == 1){
								f_copie_plateau(plateau, plateauFictif);
								f_bouge_piece(plateauFictif, i, j, k, l, 0);
								val=f_max(plateauFictif, profondeur-1, i, j, k , l);
								if(val.cout < min_val.cout){
									min_val=val;
								}
							}
						}
					...
					for( l=-1; l<=1; l++){
						if(k!=0 && l!=0){
							...
							if(result == 1){
								f_copie_plateau(plateau, plateauFictif);
								f_bouge_piece(plateauFictif, i, j, k, l, 0);
								val=f_min(plateauFictif, profondeur-1, i, j, k, l);
								if(val.cout > max_val.cout){
									max_val=val;
								}
						}
						}
...
int f_IA( int joueur){
	...
	printf("dbg: entering %s %d\n", __FUNCTION__, __LINE__);
#endif
	Coup w = f_max(plateauDeJeu, PROFONDEUR, 0, 0, 0, 0);
	f_bouge_piece(plateauDeJeu, w.l1, w.c1, w.l2, w.c2, joueur);
	int result = f_eval(plateauDeJeu);
