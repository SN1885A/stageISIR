...
double dyna_MG(double theta[PHI_SIZE_SIZE], double b[PHI_SIZE_SIZE], double F[PHI_SIZE_SIZE][PHI_SIZE_SIZE], PQueue pQueue, double Q[GRID_SIZE][GRID_SIZE][NB_ACTIONS], int X, int Y, int A,  int* step_to_converge){
	...
	double phi1AlphaDelta[PHI_SIZE];
	...
	for(e=0; e<NB_EPISODES; e++){
		...
		for (pas=0; pas<NB_STEPS; pas++){
			...
			//delta updating
			//multiplicationMatrix(phi2, F, phi1, 2, 2, 1);
			r = multiplicationMatrixOneValue(phi1, b); 
			delta = r + GAMMA*multiplicationMatrixOneValue(theta, phi2)-multiplicationMatrixOneValue(theta, phi1);
		
			//theta updating
			multiplicationMatrixScalar(phi1AlphaDelta, phi1, ALPHA*delta);
			additionMatrix(theta, theta, phi1AlphaDelta);

			//b updating
			double tmp = multiplicationMatrixOneValue(b, phi1); 
			...
			while(pQueue != NULL){
				...
				for(j=0; j<PHI_SIZE; j++){
					if(F[indice][j] != 0){
						unitBasisVect[j] = 1;
						multiplicationMatrix2(tmp, theta, F, 1, PHI_SIZE, PHI_SIZE);
